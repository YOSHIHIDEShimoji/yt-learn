...
YouTubeチャンネルの動画を自動文字起こしし、**ローカルLLM(Ollama)** で文字起こし・要約を蓄積するツール。
...
echo "LOCAL_LLM_MODEL=qwen2.5:14b" >> .env

# ポータルのチャットで Gemini を選ぶ場合のみ任意で設定
echo "GEMINI_API_KEY=your_key_here" >> .env
```

### ローカルLLM（Ollama）の利用

パイプライン（文字起こし・要約）は Ollama 専用。`LOCAL_LLM_URL` が必須。

**Macから実行する場合** — WindowsのOllamaをTailscale経由で直接参照。SSHトンネル不要。
...
**WSLから実行する場合** — WindowsのOllamaにlocalhost経由で接続。トンネル不要。
...
| **要約** | Ollama（LOCAL_LLM_URL必須） | Ollama（LOCAL_LLM_URL必須） |
...
python src/transcribe.py sync

# transcripts/ だけ同期
python src/transcribe.py sync --only transcripts

# summaries/ だけ同期
python src/transcribe.py sync --only summaries
```
...
```bash
# チャンネル追加（言語省略時は ja）
python src/transcribe.py add "メンタリスト DaiGo" https://www.youtube.com/@mentalistdaigo
python src/transcribe.py add 3Blue1Brown https://www.youtube.com/@3blue1brown en

# チャンネル削除
python src/transcribe.py remove "メンタリスト DaiGo"

# 登録チャンネル一覧
python src/transcribe.py list
```
...
python src/transcribe.py process https://youtu.be/xxx --model tiny

# 処理済みでも強制的に再文字起こし
python src/transcribe.py process https://youtu.be/xxx --force

# チャンネル指定あり → transcripts/メンタリスト DaiGo/ に保存
python src/transcribe.py process https://youtu.be/xxx --channel "メンタリスト DaiGo" --model tiny

# 複数URL同時
python src/transcribe.py process https://youtu.be/aaa https://youtu.be/bbb --channel "メンタリスト DaiGo" --model small

# URLファイルから読み込み（URL | en で言語指定可、# はコメント）
python src/transcribe.py process -f urls.txt --channel ひろゆき

# 出力先を完全に指定
python src/transcribe.py process https://youtu.be/xxx -o ~/Desktop/output --model tiny
```
...
```bash
# 人気順で上位5本（動作確認用）
python src/transcribe.py channel "メンタリスト DaiGo" --sort popular --limit 5 --model tiny

# 人気順で上位100本
python src/transcribe.py channel "メンタリスト DaiGo" --sort popular --limit 100

# 2回目は自動で未処理の次の100件が対象になる
python src/transcribe.py channel "メンタリスト DaiGo" --sort popular --limit 100

# 再生数取得を先頭50件に絞ってソート（大規模チャンネルの高速化）
python src/transcribe.py channel "メンタリスト DaiGo" --sort popular --popular-sample 50 --limit 10 --model tiny

# 再生数キャッシュのみ構築（文字起こしなし）
python src/transcribe.py channel "メンタリスト DaiGo" --sort popular --cache-only

# 全チャンネルを人気順20本ずつ
python src/transcribe.py all --sort popular --limit 20

# 全チャンネルのキャッシュのみ一括構築
python src/transcribe.py all --sort popular --cache-only

# 処理済みでも強制的に再文字起こし（channel / all 両方対応）
python src/transcribe.py channel "メンタリスト DaiGo" --sort popular --limit 5 --force
...
```bash
# 全チャンネル一括
python src/summarize.py all

# 処理済みを無視して全件再生成
python src/summarize.py "メンタリスト DaiGo" --force
```
