...
### `src/transcribe.py` — コアエンジン（触るときは慎重に）

```bash
python src/transcribe.py channel "チャンネル名" --sort popular --limit 10 --model large-v3
python src/transcribe.py all --sort popular --limit 10 --model large-v3
...
### `src/summarize.py` — 要約エンジン（Ollama専用）
...
詳細は `Archive/` フォルダと `docs/DESIGN.md` を参照。
...
```bash
# autonomous セッションの実績比較
grep '\[session-end\]' logs/autonomous/*.log

# 直近のループ状態
tail -30 logs/autonomous/*.log
```
...
#### 検証手順
LIBRARY タブは `transcripts/` 実体が WSL にのみ存在するため **WSL 側で検証すること**（`--local` では transcripts がないため空になる）。
1. WSL で `./portal.sh` を起動
2. LIBRARY タブを開く
