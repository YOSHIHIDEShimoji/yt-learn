...
## 既知の不具合・やり残し（Phase 1 完了に必要なもの）✅ すべて解消済み
...
### 2. LOGS タブのリロードボタンがない ✅ 解消（commit 14aab5d）

`card-title-row` に変えて ↻ 更新ボタン追加 + `window.reloadLogs` 実装済み。

### 3. HOME チャンネル一覧のリロードボタンがない ✅ 解消（commit 14aab5d）

`card-title-row` に変えて ↻ 更新ボタン追加 + `window.reloadChannels` 実装済み。
...
## Phase 1 完了（2026-05-21）

1. ✅ `loadChannels()` にリロードボタン追加
2. ✅ `loadLogs()` にリロードボタン追加
3. → `main` へのマージはユーザー確認後に実施
