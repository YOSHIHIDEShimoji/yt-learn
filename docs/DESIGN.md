...
> `python src/transcribe.py all --sort popular --limit 10 --model tiny` を WSL で実行して
...
```bash
python src/transcribe.py all --sort popular --limit 10 --model tiny
```
...
| ファイル | 役割 |
|----------|------|
| `src/transcribe.py` | メインスクリプト。全ロジックここに集約 |
| `autonomous.sh` | **自律ループ（推奨）** DL/文字起こし並列、rate-limit自動回復 |
| `Archive/loop_transcribe.sh` | 直列ループ（**非推奨**・autonomous.sh に統合済み） |
| `Archive/benchmark.sh` | パラメータ最適化ツール（**非推奨**・履歴保存目的） |
| `src/summarize.py` | AI要約エンジン（Ollama専用） |
...
古い場合は手動で更新:
```bash
python src/transcribe.py refresh-cookies
```
...
```bash
python src/transcribe.py all --sort popular --limit 10 --model tiny 2>&1 | tee /tmp/yt-wsl-test.log
```
...
- DL が全チャンネルを一周するたびに `src/summarize.py all` を実行してサマリーを更新
...
| 手動更新 | `python src/transcribe.py refresh-cookies` |
